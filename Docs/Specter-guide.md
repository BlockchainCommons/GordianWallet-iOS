...
This is a guide for creating a 2-of-3 multisig wallet with Specter and FullyNoded 2.  It will show you how to add devices to Specter, create a multisig wallet, and build an unsigned PSBT with Specter, scanning the Specter PSBT QR code to sign it with FullyNoded 2 and broadcast with your node over Tor. 
...
### 1. Add device #1 to Specter

You can think of devices as signers. In this case, we will add the three xpubs from a FullyNoded 2 multisig wallet.

![](../Images/Specter/1_adddevice.png)

### 2. Open FN2, get your xpubs by going to the wallets tab, activating a 2-of-3 wallet, and tapping the "info" button to export the "device seed info"

This will allow you to export the wallet's public key descriptors. For an explanation of exactly what this descriptor represents please read [this explainer](../Docs/Electrum-guide.md#11-come-back-to-fn2-and-export-your-1st-public-key-descriptor-which-holds-all-the-xpubs-you-need-you-can-get-this-by-going-to-the-wallets-tab-enabling-the-electrum-wallet-and-tapping-the-info-button-to-export-the-devices-seed-and-public-key-descriptors) from the [Electrum guide](../Docs/Electrum-guide.md).
...
```
wsh(sortedmulti(2,[82be8e74/48'/1'/0'/2']tpubDEYij9WndcWU4ApaSz68RitBMrZRTfShsXn4qw1izEaFScR5dnP4dz1CzgmfT5iTrNeZJhMXieg2BzhCFNxrWtvaTerBio3VbFoSDixs4yR/0/*,<br>[5222e39c/48'/1'/0'/2']tpubDEQznV4Xs6BP1A1HXzogRsUiFxDsyaqLzphZmjmwrrERz9aHszb2juQzYBx9xuXymba5kkQvR76m218JiXM1DsLcgPryGCDs5P1geHoxVrx/0/*,<br>[81202613/48'/1'/0'/2']tpubDEcrpYzpqTJyhy5bzsojvL8VMrcFf4DVU7q43fuq6JhhNyxCqLzhppiUsMHAXUJv4XTnmAiezNAzdfTdg2FFefbzNh8YzN6Wv2zEYwcCC22/0/*))#czsstq8d
```
...
We need to do this because FullyNoded 2 works directly with Bitcoin Core and wants to maintain 100% compatibility with your node via the command line and console. Whereas Specter is clever and knows how to import what it needs with the minimum amount of information.

The descriptor will now look like:

```
[82be8e74/48'/1'/0'/2']tpubDEYij9WndcWU4ApaSz68RitBMrZRTfShsXn4qw1izEaFScR5dnP4dz1CzgmfT5iTrNeZJhMXieg2BzhCFNxrWtvaTerBio3VbFoSDixs4yR,<br>[5222e39c/48'/1'/0'/2']tpubDEQznV4Xs6BP1A1HXzogRsUiFxDsyaqLzphZmjmwrrERz9aHszb2juQzYBx9xuXymba5kkQvR76m218JiXM1DsLcgPryGCDs5P1geHoxVrx,<br>[81202613/48'/1'/0'/2']tpubDEcrpYzpqTJyhy5bzsojvL8VMrcFf4DVU7q43fuq6JhhNyxCqLzhppiUsMHAXUJv4XTnmAiezNAzdfTdg2FFefbzNh8YzN6Wv2zEYwcCC22
```
...
When you create the multisig in FullyNoded 2 the xpubs represent three "devices" or "signers"
...
3. The third xpub is derived from your nodes seed (which as far as FN2 is concerned is deleted forever once the keys are imported into your node). You can think of this xpub as representing your node as a signer:
...
### 6. For now FullyNoded 2 only supports 2-of-3, so lets make it a 2-of-3
...
### 8. ⚠️ Now the important part, click "receive" and check that the address matches what FullyNoded 2 exports ⚠️

![](../Images/Specter/8_seeaddress.png)

### 9. Open FullyNoded 2 and tap the "eye" icon, which derives your addresses locally, so you can verify they match.
...
### 10. This is the same wallet we used in the Electrum guide so it already has transaction history

You should verify the transactions match up in FullyNoded 2, Specter, and anywhere else you have used this wallet. 
...
### 12. Scan the PSBT QR code with FullyNoded 2 to sign it and broadcast it

* If FullyNoded 2 can not sign for all of the required signatures to spend the PSBT then it will simply sign what it can and export the PSBT in a few different formats for you to pass on to other signers. 
...
Not only can you import FullyNoded 2 wallets into Specter, but you can also create wallets with other devices on Specter, then import those wallets into Fully Noded simply by clicking "Settings" in Specter and scanning the "wallet import/export" QR code. **This currently only works for 2-of-3 bech32 multisig wallets as far as FullyNoded 2 is concerned. More wallet types can be supported in the future. Consider sponsoring us to continue further development.**

![](../Images/Specter/18_importwallet.png)

Try it yourself simply by scanning this QR code with Fully Noded 2. You will need to go to "wallet tools" and rescan the blockchain for the transactions to show up.
