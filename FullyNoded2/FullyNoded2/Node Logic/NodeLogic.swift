...
class NodeLogic {
        ...
        reducer.makeCommand(walletName: "", command: .getnetworkinfo, param: "") {
            ...
            if !reducer.errorBool {
                ...
                if reducer.dictToReturn != nil {
                    ...
                    completion((false, nil, "returned value was nil"))
                    
                }
                
            } else {
                
                completion((false, nil, "\(reducer.errorDescription)"))
    ...
    func loadWalletData(wallet: WalletStruct, completion: @escaping ((success: Bool, dict: [String:Any]?, errorDescription: String?)) -> Void) {
        ...
        let reducer = Reducer()
        reducer.makeCommand(walletName: wallet.name, command: .listunspent, param: "0") { [unowned vc = self] in
                    ...
                    if let blockchainInfo = reducer.dictToReturn {
                        ...
                        if let chain = blockchainInfo["chain"] as? String {
                            ...
                            if chain == "main" {
                                ...
                                cd?.updateEntity(id: node.id, keyToUpdate: "network", newValue: "mainnet", entityName: .nodes) {_ in }
                                
                            } else if chain == "test" {
                                
                                cd?.updateEntity(id: node.id, keyToUpdate: "network", newValue: "testnet", entityName: .nodes) {_ in }
    ...
    func loadTransactionData(wallet: WalletStruct, completion: @escaping ((success: Bool, array: [[String:Any]]?, errorDescription: String?)) -> Void) {
        ...
        let reducer = Reducer()
        reducer.makeCommand(walletName: wallet.name, command: .listtransactions, param: "\"*\", 50, 0, true") { [unowned vc = self] in
    ...
    func parseUtxos(wallet: WalletStruct, utxos: NSArray, completion: @escaping ((success: Bool, dict: [String:Any]?, errorDescription: String?)) -> Void) {
            ...
            if let utxoDict = utxo as? NSDictionary {
                ...
                if let desc = utxoDict["desc"] as? String {
                    ...
                    if wallet.index <= index {
                        ...
                        cd?.updateEntity(id: wallet.id, keyToUpdate: "index", newValue: index + 1, entityName: .wallets) { (success, errorDescription) in
        ...
        cd?.updateEntity(id: wallet.id, keyToUpdate: "lastBalance", newValue: amount, entityName: .wallets) { _ in

            cd?.updateEntity(id: wallet.id, keyToUpdate: "lastUsed", newValue: Date(), entityName: .wallets) { _ in

                cd?.updateEntity(id: wallet.id, keyToUpdate: "lastUpdated", newValue: Date(), entityName: .wallets) { _ in
